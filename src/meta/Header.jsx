    ...
    <>
      ...
      <div className="text-sm fixed top-0 left-0 z-100 w-full h-[70px] flex items-start justify-center select-none px-5 py-4">
        <motion.header
          ...
          className="w-full max-w-3xl flex flex-wrap items-start justify-between bg-neutral-900/85 backdrop-blur-sm rounded-3xl text-white overflow-hidden shadow-md"
        >
          <div
            data-action="hero"
            onClick={GetScrollSection}
            className="w-[calc(100%-62px)] h-[46px] md:w-[200px] flex items-center justify-start px-3.5 cursor-pointer"
          >
            {switchToKawaiLogo ? (
              <div className="px-2 py-2 h-[46px] pointer-events-none">
                <img
                  src="/icon/kawai-logo-ff66a842.svg"
                  alt="TKJ Skanesga Kawai Logos"
                  className="w-full h-full pointer-events-none"
                />
              </div>
            ) : (
              <div className="flex items-center pointer-events-none">
                <b className="font-bold ml-2 pointer-events-none">
                  TKJ Skanesga
                </b>
              </div>
            )}
          ...
          <div className="md:hidden w-[62px] h-[46px] flex items-center justify-center p-1 pr-2">
            <button
              ...
              className="w-[36px] h-[36px] flex items-center justify-center p-2 cursor-pointer"
          ...
          <nav className="w-full md:w-[calc(100%-200px)] max-md:pb-3 max-md:mt-1 h-full md:h-[46px] flex flex-wrap items-center justify-end px-3.5 origin-top">
            {linkList.map((link, index) => (
              <button
                ...
                onClick={GetScrollSection}
                className="p-1 px-2.5 hover:text-neutral-400 rounded-2xl transition-all duration-300 cursor-pointer max-md:block max-md:w-full max-md:py-2 text-left"
                aria-label={`Jump To ${link.action}`}
              >
                {link.label}
              </button>
            ))}
