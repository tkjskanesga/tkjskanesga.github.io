...
export default function Header() {
  ...
  return <>
    ...
    <div className="text-sm fixed top-0 left-0 z-100 w-full h-[70px] flex items-center justify-center select-none px-5">
      <header className="w-full max-w-4xl h-[46px] flex items-center justify-between bg-neutral-300 rounded-3xl">
        <div data-action="hero" onClick={GetScrollSection} className="w-[200px] h-full flex items-center justify-start px-3.5 cursor-pointer">
          ...
          />
          <b className="font-bold text-neutral-900 ml-2 pointer-events-none">TKJ Skanesga</b>
        </div>
        <nav className="w-[calc(100%-200px)] h-full flex items-center justify-end px-3.5">
          {linkList.map((link, index) => (
            <a key={index} data-action={link.action} onClick={GetScrollSection} className="p-1 px-2.5 text-neutral-900 hover:text-neutral-600 rounded-2xl transition-all duration-300 cursor-pointer">{link.label}</a>
          ))}
        </nav>
      </header>
    </div>
